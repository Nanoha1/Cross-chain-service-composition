...
# Read medical data
with open('/tmp/medical-data-30.json', 'r', encoding='utf-8-sig') as f:
    records = json.load(f)

container = "xuperchain-sub3-node1"
rpc_port = 37104
chain_name = "subchain3"
...
print("Writing 30 medical data records to subchain3...")
...
for record in records:
    try:
        ...
        cmd = f'cd /home/xchain && echo "{b64}" | base64 -d > /tmp/args.json && ARGS=$(cat /tmp/args.json) && ./bin/xchain-cli native invoke --method Set -a "$ARGS" {contract_name} --fee 400 --keys {keys_path} -H 127.0.0.1:{rpc_port} --name {chain_name}'
